...
8. `CombatManager` continuously organizes available land, air, and naval reinforcements around the selected objective, keeps only a small offensive reserve, and maintains T3 mobile garrisons at forward bases.
...
Enemy unit observations originate from `AIBrain:GetUnitsAroundPoint` around a bounded rotating set of friendly observers. Records decay and expire. Directors may use enemy starting positions because they are fixed scenario/lobby information, but they do not enumerate enemy-brain units, inspect enemy economy, or enable omni vision.
